"""Command-line entry point for vibechess."""
...
from vibechess.protocols.gui import GuiAiConfig, GuiConfig, parse_ai_config, run_gui_loop
from vibechess.protocols.uci import UciConfig, run_uci_loop
from vibechess.ui.terminal import HumanQuit, PlayConfig, play_terminal


def build_parser() -> argparse.ArgumentParser:
    """Build the vibechess argument parser."""
    parser = argparse.ArgumentParser(
        prog="vibechess",
        description="vibechess command-line tools.",
    )
    parser.add_argument(
        "--version",
        action="version",
        version=f"vibechess {__version__}",
    )
    ...
    gui_server = subparsers.add_parser(
        ...
        description=(
            "Run the vibechess GUI backend. The protocol reads one JSON request per "
    ...
    *,
    ...
    if args.command == "play":
        ...
        try:
            ...
            print(f"vibechess play: {exc}", file=error_stream)
            return 2
        return 0
    ...
    if args.command == "gui-server":
        try:
            ...
            print(f"vibechess gui-server: {exc}", file=error_stream)
            return 2
        return 0
