...
from vibechess.engine.legal_moves import has_legal_move, is_in_check, legal_moves
...
from vibechess.engine.square import FILES, RANKS, Square, file_index, parse_square, rank_index
...
class _PgnParserState:
    ...
    This uses the shared engine transition helpers so parsing can reuse the SAN
    legal-move tuple without allocating immutable ``Game`` history for each ply.
    """
    ...
    def pre_move_outcome(self, legal: tuple[Move, ...]) -> Outcome | None:
        ...
        return outcome_for_state(state, legal)
...
def _square_name_from_move_target(move: Move) -> str:
    from vibechess.engine.square import square_name
