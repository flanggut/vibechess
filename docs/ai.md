...
- `vibechess.nn.self_play_dataset`: versioned compressed NPZ dataset IO containing encoded positions, legal masks, MCTS policy targets, outcome targets, metadata, and game records; the dataset tensor files and `games.jsonl` schema are shared by generated self-play and PGN ingestion shards.
