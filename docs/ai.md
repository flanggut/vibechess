...
- Self-play datasets with versioned metadata. (Generation is implemented in `tinychess.nn.self_play`; dataset IO and validation are implemented in `tinychess.nn.self_play_dataset`.)
...
- Checkpoint-vs-baseline evaluation with early smoke promotion criteria, plus checkpoint-vs-checkpoint neural evaluation without baselines or promotion gates. (Implemented in `tinychess.ai.evaluation` and `scripts/evaluate.py`.)
...
- `tinychess.nn.self_play`: self-play game generation using neural or classical MCTS labels. Neural self-play also supports an optional throughput-oriented `batch_size` mode that batches root/decision inference across independent games while keeping a separate `NeuralMCTSPlayer` tree/RNG stream per game.
- `tinychess.nn.self_play_dataset`: versioned compressed NPZ dataset IO containing encoded positions, legal masks, MCTS policy targets, outcome targets, metadata, and game records; the dataset tensor files and `games.jsonl` schema are shared by generated self-play and PGN ingestion shards.
...
- `tinychess.ai.evaluation`: player-vs-player match runner, checkpoint player loading, random/classical MCTS baseline comparisons, checkpoint-vs-checkpoint neural matches, JSON reports, and explicit early promotion criteria for baseline mode only.
