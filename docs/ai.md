...
- `tinychess.nn.train`: masked policy cross-entropy, value MSE, a tiny MLX optimizer loop, default 10% validation holdout when enough samples are available, shard-wise training support with in-memory optimizer continuity, configurable per-step `metrics.jsonl`, per-epoch loss reporting in `epoch_metrics.jsonl`, `training.json`, and `checkpoint-final` output.
