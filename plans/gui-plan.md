...
## Goal
Build a small native SwiftUI macOS application that lets a human play vibechess against random, classical MCTS, or optional checkpoint-backed neural MCTS players through a Python JSON-lines backend without rewriting chess legality in Swift.
...
- Existing Python entrypoint is `src/vibechess/cli.py` with `play` and bounded `uci` subcommands.
- Existing protocol code is `src/vibechess/protocols/uci.py`; GUI support should be a separate protocol module to avoid broadening bounded UCI semantics.
- Existing playable/AI integration exists in `src/vibechess/ui/terminal.py` and supports `RandomPlayer`, `MCTSPlayer`, and checkpoint-backed neural `NeuralMCTSPlayer` through `PlayConfig` patterns.
...
- Existing Swift package at `swift/Package.swift` only exports `VibeChessCore`; the GUI should add a macOS app target while keeping acceleration work separate.

## Target Layout

### Python backend additions
```text
src/vibechess/protocols/
├── __init__.py
...
├── README.md
├── Sources/
│   ├── VibeChessCore/              # existing acceleration bootstrap, do not add chess rules here
│   └── VibeChessMacApp/            # new executable SwiftUI app target
...
└── Tests/
    ├── VibeChessCoreTests/         # existing
    └── VibeChessMacAppTests/       # backend model/client parsing tests where feasible
```

If Swift Package Manager executable SwiftUI app packaging is awkward, create an Xcode app project under `swift/VibeChessMacApp/` but keep shared source names above. Do not place chess-rule logic in Swift; Swift may map squares, display pieces, and route UCI-like strings only.

## JSON-lines GUI Protocol

### Transport
- Backend command: `uv run vibechess gui-server` for development/local MVP.
...
1. **[Done] Define backend GUI protocol dataclasses and serializers**
   - File: `src/vibechess/protocols/gui.py`
   ...
   - Completed: implemented in `src/vibechess/protocols/gui.py` with focused coverage in `tests/test_gui_protocol.py`.

2. **[Done] Add CLI entrypoint for backend server**
   - File: `src/vibechess/cli.py`
   ...
   - Acceptance: `uv run vibechess gui-server` responds to `hello`, `state`, and `quit` JSON-lines requests; `uv run vibechess --help` lists the subcommand.
   - Completed: implemented in `src/vibechess/cli.py` with focused CLI coverage in `tests/test_cli.py`.
...
4. **[Done] Test Python AI commands**
   ...
   - Completed: implemented and tested GUI `aiMove` support for random, classical MCTS, and graceful neural missing-checkpoint failure in `src/vibechess/protocols/gui.py` and `tests/test_gui_protocol.py`.

5. **[Done] Add Swift app target skeleton**
   - File: `swift/Package.swift`
   - Changes: add an executable target `VibeChessMacApp` for macOS SwiftUI and a test target `VibeChessMacAppTests` if SPM supports the desired app shape. Keep existing `VibeChessCore` unchanged except dependency declarations if needed.
   - File: `swift/Sources/VibeChessMacApp/VibeChessMacApp.swift`
   ...
   - Completed: added the SwiftPM `VibeChessMacApp` executable target, placeholder SwiftUI root window, and focused app skeleton test.

6. **[Done] Add Swift protocol models**
   - Files: `swift/Sources/VibeChessMacApp/BackendModels.swift`, `swift/Tests/VibeChessMacAppTests/BackendModelsTests.swift`
...
7. **[Done] Add Swift backend subprocess client**
   - File: `swift/Sources/VibeChessMacApp/BackendClient.swift`
   - Changes: implement an actor or main-safe service that launches `uv run vibechess gui-server` in repo/dev mode, sends serialized requests, reads line-delimited responses, maps errors, and terminates on app close.
...
8. **[Done] Add app state/view model**
   - File: `swift/Sources/VibeChessMacApp/AppState.swift`
...
9. **[Done] Implement board UI with Unicode pieces**
   - Files: `swift/Sources/VibeChessMacApp/BoardView.swift`, `swift/Sources/VibeChessMacApp/SquareView.swift`
...
10. **[Done] Implement controls, move list, and status**
   - Files: `swift/Sources/VibeChessMacApp/ControlsView.swift`, `swift/Sources/VibeChessMacApp/MoveListView.swift`, root content view if separate.
...
11. **[Done] Wire human-vs-AI flow**
   - Files: `swift/Sources/VibeChessMacApp/AppState.swift`, `BackendClient.swift`, UI files.
...
12. **[Done] Add undo/reset behavior**
   - Files: Python `src/vibechess/protocols/gui.py`; Swift `AppState.swift`, `ControlsView.swift`.
...
13. **[Done] Documentation update**
   ...
   - Changes: document `vibechess gui-server`, protocol boundary, local Swift app launch, MVP limitations, and distributable-app packaging as later phase.
...
## Files to Modify
- `src/vibechess/cli.py` - add `gui-server` subcommand and backend launch options.
- `src/vibechess/protocols/__init__.py` - optionally export/document GUI protocol module.
...
## New Files
- `src/vibechess/protocols/gui.py` - JSON-lines GUI protocol/session/server loop.
- `tests/test_gui_protocol.py` - Python backend protocol tests.
- `swift/Sources/VibeChessMacApp/VibeChessMacApp.swift` - app entrypoint.
- `swift/Sources/VibeChessMacApp/BackendModels.swift` - Codable protocol models.
- `swift/Sources/VibeChessMacApp/BackendClient.swift` - Python subprocess JSON-lines client.
- `swift/Sources/VibeChessMacApp/AppState.swift` - main app view model/state machine.
- `swift/Sources/VibeChessMacApp/BoardView.swift` - chessboard rendering.
- `swift/Sources/VibeChessMacApp/SquareView.swift` - individual square rendering/click handling.
- `swift/Sources/VibeChessMacApp/ControlsView.swift` - controls and AI config UI.
- `swift/Sources/VibeChessMacApp/MoveListView.swift` - move history UI.
- `swift/Tests/VibeChessMacAppTests/BackendModelsTests.swift` - Swift decoding/model tests.
- `swift/Tests/VibeChessMacAppTests/AppStateTests.swift` - optional mocked-client view model tests.
...
- `uv run vibechess gui-server` assumes the app is launched from a developer environment with `uv` and the repo available. Bundled distribution requires a separate packaging slice.
...
- Should local dev backend path be auto-detected from app launch directory, user-configurable, or hardcoded to `uv run vibechess gui-server` for the MVP? Default: configurable command with a sensible dev default.
